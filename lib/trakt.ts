...
export async function pushTraktRating(userId: string, item: { kind: MediaKind; tmdbId: number; score: number }) { const entry = { rated_at: new Date().toISOString(), rating: Math.round(item.score), ids: { tmdb: item.tmdbId } }; await postSync(userId, "/sync/ratings", item.kind === "movie" ? { movies: [entry] } : { shows: [entry] }); }
...
export async function removeTraktWatchlist(userId: string, item: { kind: MediaKind; tmdbId: number }) { const entry = { ids: { tmdb: item.tmdbId } }; await postSync(userId, "/sync/watchlist/remove", item.kind === "movie" ? { movies: [entry] } : { shows: [entry] }); }
