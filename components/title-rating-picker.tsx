...
export function TitleRatingPicker({ mediaId, path, rating, signedIn }: { mediaId: number | null; path: string; rating?: number | null; signedIn: boolean }) {
  if (!mediaId || !signedIn) return <Link className="rating-picker signed-out" href="/login"><Gauge size={18} /><span><small>Your rating</small><strong>Sign in to rate</strong></span></Link>;
  return <details className="rating-picker">
    <summary><Gauge size={20} /><span><small>Your rating</small><strong>{rating ? `${Number(rating).toFixed(1)} / 10` : "Rate this title"}</strong></span></summary>
