...
export function ReviewComposer({ targetType, targetId, path, currentRating }: Props) {
  ...
  return <form className="review-composer" action={action}>
    ...
    <div className="review-composer-top"><div><span className="eyebrow">Your score</span><strong>{score !== null && Number.isFinite(score) ? <>{score.toFixed(1)}<small>/10</small></> : "Review without a rating"}</strong></div><button type="button" className="rating-clear" onClick={() => setScore(null)}>Clear rating</button></div>
    <div className="review-score-slider"><input type="range" min="1" max="10" step="0.1" value={score ?? 5.5} onChange={event => setScore(Number(event.target.value))} aria-label="Review rating from 1 to 10" /><div><span>1.0</span><span>5.5</span><span>10.0</span></div></div>
