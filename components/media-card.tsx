...
import { Check, ExternalLink, Heart, ListPlus, Star } from "lucide-react";
import { useActionState, useRef, useState } from "react";
import { quickTrack, type ListActionState } from "@/app/actions/library";
...
export function MediaCard({ item }: { item: MediaSummary }) {
  ...
  const startHold = () => { holdTimer.current = setTimeout(() => setMenu(true), 520); };
  ...
  return <article className="media-card interactive-card" onContextMenu={event => { event.preventDefault(); setMenu(true); }} onPointerDown={event => { if (event.pointerType === "touch") startHold(); }} onPointerUp={stopHold} onPointerCancel={stopHold} onPointerLeave={stopHold}>
