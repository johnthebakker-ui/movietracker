...
export function MediaCard({ item, listContext, progressLabel }: { item: MediaSummary; listContext?: ListContext; progressLabel?: string }) {
  ...
  const popup = menu && typeof document !== "undefined" ? createPortal(<div className="media-context-menu" role="menu" ref={menuRef} style={{ top: position.top, left: position.left, maxHeight: position.maxHeight }}>
    ...
    <div className="context-list-section"><span>Add to another list</span>{lists === null && !listError ? <small>Loading your lists…</small> : listError ? <Link href={listError.startsWith("Sign in") ? "/login" : "/lists"}>{listError}</Link> : lists?.length ? <><label className="context-list-search"><Search size={14} /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Find a list" aria-label="Find a list" /></label><div className="context-list-options">{filtered.map(list => <form action={addAction} key={list.id}>{hidden}<input type="hidden" name="listId" value={list.id} /><button disabled={adding || list.contains} className={list.contains ? "contained" : ""}><span className="context-list-name">{list.name}</span><span className="context-list-action">{list.contains ? <><Check size={14} /> Added</> : <><ListPlus size={14} /> Add</>}</span></button></form>)}</div></> : <Link href="/lists">Create your first list</Link>}</div>
    ...
    {[state, addState, removeState].map((result, index) => result.status !== "idle" && <span className={`quick-action-result ${result.status}`} role="status" key={index}>{result.status === "success" ? "✓" : "!"} {result.message}</span>)}
