...
import { getCollection, getMedia, imageUrl } from "@/lib/tmdb";
...
export default async function TitlePage({ params }: Props) {
  ...
  const startYear = yearOf(item.releaseDate); const endYear = yearOf(item.endDate); const showEnded = ["Ended", "Canceled", "Cancelled"].includes(item.status); const showRun = kind === "show" ? showEnded ? `${startYear}${endYear && endYear !== startYear ? `–${endYear}` : ""}` : `${startYear}–present` : "";
  const statusOptions = [{ value: "planned", label: watched ? "Plan rewatch" : "Plan", icon: BookmarkPlus }, { value: "watching", label: "Watching", icon: Eye }, { value: "completed", label: "Watched", icon: Check }, { value: "paused", label: "Paused", icon: CirclePause }, { value: "dropped", label: "Dropped", icon: CircleX }] as const;
  return <main className="page">
    <section className="detail-hero">
      ...
      <div className="shell detail-layout">
        ...
        <div className="detail-copy"><div className="eyebrow">{kind === "movie" ? "Film" : "Television series"}</div><h1 className="display">{item.title}</h1>
          <div className="meta-line"><span>{kind === "show" ? showRun : yearOf(item.releaseDate)}</span><span>{minutesToLabel(item.runtime)}</span><span>{item.status}</span></div>
    ...
    <div className="shell">
      ...
      {mediaId && user && <section className="section review-section"><div className="section-head"><div><div className="eyebrow">Your take</div><h2 className="display">{existingReview ? "Edit your review" : "Write a review"}</h2></div><p>One score, one opinion. Changing it here also updates your rating above.</p></div><ReviewComposer targetType="media" targetId={mediaId} path={path} currentRating={rating?.score} existingReview={existingReview} /></section>}
      ...
      {ratedCollection.length > 0 && <MediaSection eyebrow="Continue the collection" title={`More from ${item.collectionName?.replace(/ Collection$/i, "") ?? "this franchise"}`} items={ratedCollection}/>}
