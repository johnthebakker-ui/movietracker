...
export default async function EpisodePage({ params }: { params: Promise<{ kind: string; id: string; season: string; episode: string }> }) {
  ...
  if (supabase && episodeId) {
    const [{ data: reviewRows }, { data: ratingRows }, watch] = await Promise.all([
      supabase.from("reviews").select("id,title,body,contains_spoilers,created_at,rating_id,profiles(username,display_name,avatar_url),ratings(score)").eq("episode_id", episodeId).order("created_at", { ascending: false }).limit(20),
