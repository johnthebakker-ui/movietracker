...
export default async function EpisodePage({ params }: { params: Promise<{ kind: string; id: string; season: string; episode: string }> }) {
  ...
  if (supabase && episodeId) {
    const [{ data: reviewRows }, { data: ratingRows }, watch] = await Promise.all([
      supabase.from("reviews").select("id,user_id,title,body,contains_spoilers,created_at,updated_at,rating_id,profiles(username,display_name,avatar_url),ratings(score)").eq("episode_id", episodeId).order("created_at", { ascending: false }).limit(20),
  ...
  return <main className="page episode-detail-page">
    ...
    <div className="shell">{episodeId && <TargetRatingBar targetType="episode" targetId={episodeId} path={path} signedIn={Boolean(user)} rating={communityRating} userRating={userRating} sources={[...(episode.voteAverage ? [{ source: "TMDB", value: `${episode.voteAverage.toFixed(1)}/10` }] : []), ...externalRatings]} />}<TitleMedia videos={episode.videos ?? []} images={episode.images ?? []} />
      {(episode.cast?.length ?? 0) > 0 && <section className="section"><div className="section-head"><div><div className="eyebrow">Guest stars & cast</div><h2 className="display">In this episode</h2></div></div><div className="people-grid">{episode.cast!.slice(0, 12).map(person => <Link className="person-card" href={`/person/${person.id}`} key={`${person.id}-${person.character ?? "cast"}`}><div>{person.profile_path ? <Image className="person-photo" src={imageUrl(person.profile_path, "w300")!} width={240} height={240} alt="" /> : <div className="person-photo" />}</div><div className="person-name">{person.name}</div><div className="person-role truncate">{person.character}</div></Link>)}</div></section>}
      {episodeId && <TargetReviewSections targetType="episode" targetId={episodeId} path={path} signedIn={Boolean(user)} userRating={userRating} reviews={reviews} currentUserId={user?.id} />}
    </div>
  </main>;
}
