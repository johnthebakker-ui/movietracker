...
export default async function PersonPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ kind?: string; year?: string }> }) {
  ...
  const credits = await withCommunityRatings(person.credits.filter(item => !filters.kind || item.kind === filters.kind).filter(item => !filters.year || item.releaseDate?.startsWith(filters.year)));
