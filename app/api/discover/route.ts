...
export async function GET(request: Request) {
  ...
  try { const data = await discover(kind, options); return NextResponse.json({ ...data, items: await withCommunityRatings(data.items) }); } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Discovery failed" }, { status: 502 }); }
}
