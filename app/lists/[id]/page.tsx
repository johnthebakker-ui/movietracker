...
export default async function ListDetail({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ sort?: string; group?: string }> }) {
  ...
  let rows: Row[] = (items ?? []).map((row: any) => ({ ...row, media: Array.isArray(row.media) ? row.media[0] : row.media })).filter((row: any) => row.media).map((row: any) => ({ ...row, item: fromDbMedia(row.media) }));
  ...
  const isOwner = userData.user?.id === list.user_id; const featured = rows.find(row => row.media?.id === list.featured_media_id)?.media; const heroArt = list.cover_url || imageUrl(featured?.backdrop_path || featured?.poster_path, "original"); const owner = Array.isArray(list.profiles) ? list.profiles[0] : list.profiles;
  const groups = new Map<string, Row[]>(); if (grouped) rows.forEach(row => { const key = row.media.kind === "movie" && row.media.collection_tmdb_id ? row.media.collection_name || "Collection" : "Other titles"; groups.set(key, [...(groups.get(key) ?? []), row]); });
