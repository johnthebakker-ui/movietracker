...
import { discoveryApiFilters, discoverCatalog, filterPersonalDiscoveryItems } from "@/lib/catalog-discovery";
...
export default async function Discover({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const filteredItems = await filterPersonalDiscoveryItems(data.items, params, supabase, user?.id);
  const genres = await getGenres(data.format === "all" ? undefined : data.format); const ratedItems = await withCommunityRatings(filteredItems, supabase);
