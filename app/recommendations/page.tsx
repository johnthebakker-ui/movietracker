...
type Filters = { kind?: string; genre?: string; country?: string; year?: string; hideWatched?: string; hideListed?: string; hideAnimation?: string; excludeGenres?: string; shuffle?: string };
export default async function Recommendations({ searchParams }: { searchParams: Promise<Filters> }) {
  ...
  await ensureRecommendations(user.id); const filters = { kind: normalizedParams.kind, genre: params.genre, country: params.country, year: params.year, hideWatched: params.hideWatched !== "0", hideListed: params.hideListed !== "0", hideAnimation: params.hideAnimation === "1", excludeGenres: params.excludeGenres, shuffle: params.shuffle }; const [genres, result] = await Promise.all([getGenres(), recommendationPage(supabase, user.id, filters)]); const rated = await withCommunityRatings(result.items.map(entry => entry.item), supabase); const initial = result.items.map((entry, index) => ({ ...entry, item: rated[index] })); const query = new URLSearchParams({ ...(filters.kind ? { kind: filters.kind } : {}), ...(params.genre ? { genre: params.genre } : {}), ...(filters.country ? { country: filters.country } : {}), ...(params.year ? { year: params.year } : {}), ...(params.shuffle ? { shuffle: params.shuffle } : {}), ...(params.excludeGenres ? { excludeGenres: params.excludeGenres } : {}), hideWatched: filters.hideWatched ? "1" : "0", hideListed: filters.hideListed ? "1" : "0", ...(filters.hideAnimation ? { hideAnimation: "1" } : {}) }).toString();
